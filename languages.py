LANGUAGES = {
    "en": {
        ...
        "mcptoken_sent": "🔌 *Token to connect Claude (read-only access to your finance data)*\n\nToken: `{token}`\nURL: `{url}`\n\nFor Claude Code, run on the machine it's installed on:\n```\nclaude mcp add --transport http money-manager {url} --header \"Authorization: Bearer {token}\"\n```\n\n⚠️ This token can read all your accounts, transactions, budgets and goals — never share it. Tap below to invalidate it and issue a new one.",
        "mcptoken_regenerate_btn": "🔄 Regenerate token (invalidates the old one)",
        "mcptoken_no_url_placeholder": "https://<your-tunnel-or-domain>",
        "web_logout": "Log out",
    ...
    "ru": {
        ...
        "mcptoken_sent": "🔌 *Токен для подключения Claude (доступ только на чтение к вашим финансам)*\n\nТокен: `{token}`\nАдрес: `{url}`\n\nДля Claude Code выполните на компьютере, где он установлен:\n```\nclaude mcp add --transport http money-manager {url} --header \"Authorization: Bearer {token}\"\n```\n\n⚠️ Этот токен даёт доступ на чтение ко всем вашим счетам, транзакциям, бюджетам и целям — никому его не показывайте. Кнопка ниже аннулирует его и выдаст новый.",
        "mcptoken_regenerate_btn": "🔄 Обновить токен (старый перестанет работать)",
        "mcptoken_no_url_placeholder": "https://<ваш-туннель-или-домен>",
        "web_logout": "Выйти",
