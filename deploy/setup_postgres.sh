...
umask 077
{
  ...
  echo "# Run deploy/configure_env.sh to pick this up into .env, then delete this file."
...
============================================================
 ...
 Next steps:
   1. ./deploy/configure_env.sh   (picks up DATABASE_URL from ${CREDS_FILE}
                                   automatically, asks only for the bot token)
   2. venv/bin/python bot.py      (creates/migrates its schema automatically)
